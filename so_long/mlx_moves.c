...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/02 14:17:33 by fzuccaro          #+#    #+#             */
/*   Updated: 2025/02/09 17:33:58 by fzuccaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_player(t_map *map, int new_y, int new_x, int direction)
{
	...
	ft_printf("You moved again, counter is:%d\n", map->mlx->count_moves);
	ft_spawn_e(map, new_y, new_x, direction);
	ft_draw(map->mlx, map);
	return (0);
}

void	ft_spawn_e(t_map *map, int new_y, int new_x, int direction)
{
	if (map->matrix[new_y][new_x] == 'E' && map->count_c > 0)
		map->matrix[new_y][new_x] = 'P';
...
int	count_moves(t_map *map)
{
	...
	mlx_string_put(map->mlx->con, map->mlx->wp, 100, 20, 0x000000, moves);
	free(moves);
	return (0);
}
...
void	ft_draw_checkp(int x, int y, t_mlx *mlx)
{
	if (mlx->direction == 0)
		...
		mlx_put_image_to_window(mlx->con, mlx->wp, mlx->p_dx, x * 48, y * 48);
}
